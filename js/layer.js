...
function calculateLayerFeed() {
    ...
    if (dailySelected) {
        ...
        textOutput = "Your " + qty + " layer birds will consume " + inBags + " (25kg) bags daily";
    ...
    if (rangeSelected) {
        if (startWeek.trim().length == 0 || endWeek.trim().length == 0) {
            ...
            if (endWeek > 104) {
                alert("Range exceeded: maximum of 104 weeks");
                return;
            }
    ...
    if (weeklySelected) {
        ...
        textOutput = "Your " + qty + " layer birds will consume " + inBags + " (25kg) bags daily.<br>" +
...
function saveToFile() {
    ...
    // document.getElementById("save").style.visibility = "hidden";
    textOutput = "<h2 class='print'>Poultry Birds Ration Calculator (Layers)</h2><p>Thanks for using our service. You can reach out anytime and we'll be here to serve you.</p><br><br>";
